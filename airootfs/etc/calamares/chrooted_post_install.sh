...
# Remove archcraft mirror
sed -i '/archcraft/d' /etc/pacman.conf
sed -i '/SigLevel/d' /etc/pacman.conf

# Remove the launch script on startup
sed -i '/installer/d' /home/$new_user/.config/openbox/autostart
sed -i '/calamares/d' /home/$new_user/.config/openbox/autostart

# Add auto update command 
echo "pacman -Syu --noconfirm &" >> /home/$new_user/.config/openbox/autostart

sed -i '/archcraft/d' /home/$new_user/.config/openbox/themes/default/polybar/config.ini
...
Name=Getscreen" > /usr/share/applications/Getscreen.desktop
